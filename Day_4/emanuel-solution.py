...
for i in bingo_boards_list_of_strings:
    ...
    for j in data:
        if j == "": data.remove(j)
    if len(i) == 0:     #This indicates that we are currently between two boards, therefore we will do nothing
        pass            #Nothing
    else:
...
list_for_winning_row = []                               #we write in this list how many numbers must be called to fill each of the 500 rows
bingo_boards_df_length = len(bingo_boards_df)
for row in range(bingo_boards_df_length):
    called_numbers_to_fill_row = 0                      #initialize with 0 as best possible score
    current_row_list = list(bingo_boards_df.loc[row])   #row we are currently looking at as list
    for element in current_row_list:
        if called_numbers_list.index(int(element)) > called_numbers_to_fill_row:
            called_numbers_to_fill_row = called_numbers_list.index(int(element))
...
" Therefore one winning board is the board number " + str(winning_board_row) + ".")

#now which column on which bingo board will be the first full column?
list_for_winning_column = []
column_length = 5
for column in range(5):
    beginning_of_current_column = 0
    ending_of_current_column = 5
    for board in range(int(bingo_boards_df_length/column_length)):
        called_numbers_to_fill_column = 0
        current_column_list = list(bingo_boards_df.iloc[beginning_of_current_column:ending_of_current_column, column])
...
if minimum_to_fill_row < minimum_to_fill_column:
    ...
    final_winning_board = winning_board_column
...
#calculate score
all_numbers_on_final_winning_board_list = []
...
numbers_not_called_on_final_winning_board = all_numbers_on_final_winning_board_list     #initialize
sum = 0 #initialize
